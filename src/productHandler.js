...
const addNewProduct = async (request, h) => {
    ...
    if (productValidation) {
        ...
        await client.query(`insert into products(id_user, title, price, amount, 
            description, imageurl, locationseller, waktupanen, kategori) 
       values($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, [`${idUser}`, `${title}`,
       `${price}`, `${amount}`, `${desc}`, `${imgUrl}`, `${locSeller}`, `${waktuPanen}`, `${kategori}`]);
